...
\subsection{Training Density Sweet Spot}

We further investigate the interaction between template count and per-template sample density. Fixing micro-language syntax to v2's grammar, we train three variants with different per-template sample counts:

\begin{table}[h]
\centering
\begin{tabular}{lccc}
\toprule
\textbf{Model} & \textbf{/Template} & \textbf{MIC} & \textbf{Behavior} \\
\midrule
v4-20x50 & 50 & 29\% & Reasoning correct, format broken \\
\textbf{v2} & \textbf{100} & \textbf{36\%} & \textbf{Both reasoning and format aligned} \\
v3-fixed & 143 & 3\% & Format correct, reasoning memorized \\
\bottomrule
\end{tabular}
\caption{Sample density sweet spot. At 50/template, entity identification succeeds (e.g. ``乙=书包'') but the answer format fails. At 143/template, the answer format is learned but reasoning collapses into template memorization.}
\label{tab:density}
\end{table}

Analysis of v4-20x50 outputs reveals a striking dissociation: the model correctly identifies entities (outputting ``乙=书包''), but fails to produce the target answer format (``书包里''). This indicates that \textbf{symbolic reasoning and output formatting are independent learning objectives} competing for limited sample budget in 64M models. At 50 samples/template, reasoning is acquired but formatting is not; at 143 samples/template, format is overlearned while reasoning degrades into memorization. The convergence sweet spot of $\sim$100 samples/template suggests a hard architectural constraint on how many distinct capabilities a 64M model can acquire from a fixed data budget.
...
By declaring entity-to-symbol mappings before reasoning, a 64M model achieves entity tracking accuracy that would otherwise require substantially larger models on raw text.

More broadly, our experiments reveal that \textbf{small-model symbolic reasoning is exquisitely sensitive to training data structure}, with clear sweet spots along three dimensions: binding format (entity mapping declarations contribute +30pp), template diversity (20 templates outperform 8 and 35), and per-template sample density ($\sim$100 samples/template is optimal). The dissociation between reasoning and output format at 50 samples/template provides direct evidence that these are independent learning objectives competing for a shared parameter budget.
...
We show that explicit object binding via a micro-language enables entity tracking in 64M models, with cross-template generalization improving from a 20\% baseline to 36\% at the optimal training configuration. Our experiments identify critical sensitivity to three structural dimensions—binding format, template diversity, and per-template sample density—and provide evidence that symbolic reasoning and output formatting are separable learning objectives competing for shared parameter capacity. These findings suggest that structured input representations can serve as a ``shortcut'' to emergent reasoning, with clear design principles for sample-efficient small language models.
