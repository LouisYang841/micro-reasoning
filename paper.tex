...
Using a 64M-parameter MiniMind-3 model fine-tuned on 500 examples of a custom micro-language with entity mapping declarations (e.g., \texttt{甲=小明 乙=苹果}), we improve entity tracking accuracy from 20\% to 83\%.
...
\begin{tabular}{lccc}
\toprule
\textbf{Condition} & \textbf{Params} & \textbf{Optimizer} & \textbf{Accuracy} \\
\midrule
Baseline (raw)      & 64M  & ---    & 20\% \\
Ablation (no map)   & 64M  & AdamW  & 53\% \\
\textbf{Full (with map)} & \textbf{64M} & \textbf{AdamW} & \textbf{83\%} \\
15-epoch (overfit)  & 64M  & AdamW  & 47\% \\
\bottomrule
\end{tabular}
\caption{Entity tracking accuracy across conditions. The entity mapping line alone contributes +30pp.}
\label{tab:results}
\end{table}

The full model with entity mapping achieves 83\% accuracy, a 4$\times$ improvement over the unfine-tuned baseline (20\%).
The ablation model—trained identically but without the entity mapping line—reaches only 53\%, confirming that explicit object binding contributes 30 percentage points to the gain.
